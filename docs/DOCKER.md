...
- Kohaku
  - S3 互換ストレージから取得したログを DB で管理

- Grafana
  - ログの視覚化

これらはすべて、Docker Compose で構築します。

外部 S3 互換ストレージを利用する `compose.external-s3.yml` の場合、`RustFS` は起動しません。

## 設定

`.env` ファイルで、Sora の log ディレクトリのパスや RustFS の設定を行います。

設定項目は `.env.common.template`、`.env.docker.template` に用意してありますので、これらを利用して設定します。

```bash
cat .env.common.template .env.docker.template > .env
```

作成した `.env` 内の設定項目はコメントアウトされています。Docker Compose の設定に必要な項目のコメントアウトを外したうえで、環境に合わせて値を設定してください。
...
- `GF_SECURITY_ADMIN_USER` - Grafana の管理ユーザー
- `GF_SECURITY_ADMIN_PASSWORD` - Grafana の管理パスワード
- `RUSTFS_BASE_DIR` - RustFS のデータ保存ディレクトリ

### 初期設定

各コンテナにマウントするディレクトリの作成と、Grafana にインストールする [Grafana DuckDB Data Source Plugin](https://github.com/motherduckdb/grafana-duckdb-datasource) の準備を行います。
...
### 外部 S3 互換ストレージを利用する場合

外部 S3 互換ストレージを利用する場合は `compose.external-s3.yml` を使用します。
...
既存の Lifecycle Management ルールを保持する必要がある場合は、この構成をそのまま使用しないでください。
...
- Docker Compose で起動した Grafana は `.env` の `GRAFANA_HTTP_PORT` で指定したポート番号、RustFS はポート番号 9000 と 9001 が公開されます。外部に公開されるサーバー上で起動させる場合は、適宜 Firewall などでアクセスを制限してください。
